...
class Compress(torch.autograd.Function):
    ...
    def forward(ctx, input, name):
        input = schemoe_custom_kernel.compress_operation(input, name)
        return input
...
class Decompress(torch.autograd.Function):
    ...
    def backward(ctx, grad):
        ...
        # print(grad)
        return schemoe_custom_kernel.compress_operation(grad, ctx.name), None
...
def a2a_ffn_overlap_forward(input, expert_fn, a2a_ffn_overlap_degree, use_2dh, group, is_compress):
    split_dim = 1
