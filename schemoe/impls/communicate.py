...
from .jit_compiler import schemoe_custom_kernel
...
def create_groups_from_world(group_count, include_init=None):
    ...
    if include_init:
        assert backend == include_init, "Only 1 backend type is allowed, get: %s v.s. %s" % (
        ...
        if ('LOCAL_RANK' not in os.environ) and ('OMPI_COMM_WORLD_SIZE' in os.environ):
            if include_init:
                dist.init_process_group(backend=backend,
                                        init_method='tcp://%s:%s' % (
                                            os.environ['MASTER_ADDR'], os.environ.get('MASTER_PORT', '23456')),
    ...
    if is_distributed:
        ...
        if dist_world_size != glob_world_size:
            ...
            for gr in range(dist_group_size):
                group_ranks = [x for x in range(
                    gr * dist_world_size, (gr + 1) * dist_world_size)]
...
def simple_all_to_all(input, group=None, background=False):
    ...
    if background:
        future_op = dist.all_to_all_single(
...
def simple_split(input, group=None):
    ...
    if world_size == 1:
        return input
    assert input.size(0) % world_size == 0, "Cannot evenly devide dim length %s into %s slices" % (
        input.size(0), world_size)
    input = input.contiguous()
...
class AllToAllStatus:
    ...
    def init(group: dist.ProcessGroup, num_split: int, split_dim: int) -> None:
        ...
        if not AllToAllStatus.initialized:
            ...
            nccl_unique_id_size = schemoe_custom_kernel.get_nccl_unique_id_size()
            nccl_unique_id = torch.zeros(
                [1, nccl_unique_id_size], dtype=torch.int8).cpu()
            if world_rank == 0:
                schemoe_custom_kernel.get_nccl_unique_id(nccl_unique_id)
            nccl_unique_id = nccl_unique_id.cuda()
            dist.broadcast(nccl_unique_id, 0, group)
            schemoe_custom_kernel.init_nccl(
                nccl_unique_id.cpu(),
                world_size,
                world_rank,
...
class CurrentStreamRelease(torch.autograd.Function):
    ...
    def forward(ctx: Any, input: Tensor, idx: int) -> Tensor:
        ...
        ctx.idx = idx
        input = input.contiguous()
        return schemoe_custom_kernel.current_stream_release(input, idx)

    @staticmethod
    def backward(ctx: Any, grad_output: Tensor) -> Tensor:
        if not AllToAllStatus.initialized:
            return (grad_output, None)
        return (schemoe_custom_kernel.current_stream_acquire(grad_output, ctx.idx), None)
...
class CurrentStreamAcquire(torch.autograd.Function):
    ...
    def forward(ctx: Any, input: Tensor, idx: int) -> Tensor:
        if not AllToAllStatus.initialized:
            return input
        ctx.idx = idx
        return schemoe_custom_kernel.current_stream_acquire(input, idx)
    ...
    def backward(ctx: Any, grad_output: Tensor) -> Tensor:
        ...
        return (schemoe_custom_kernel.current_stream_release(grad_output, ctx.idx), None)
...
class NcclStreamRelease(torch.autograd.Function):
    ...
    def forward(ctx: Any, input: Tensor, idx: int) -> Tensor:
        if not AllToAllStatus.initialized:
            return input
        ctx.idx = idx
        return schemoe_custom_kernel.nccl_stream_release(input, idx)

    @staticmethod
    def backward(ctx: Any, grad_output: Tensor) -> Tensor:
        if not AllToAllStatus.initialized:
            return (grad_output, None)
        return (schemoe_custom_kernel.nccl_stream_acquire(grad_output, ctx.idx), None)
...
class NcclStreamAcquire(torch.autograd.Function):
    ...
    def forward(ctx: Any, input: Tensor, idx: int) -> Tensor:
        if not AllToAllStatus.initialized:
            return input
        ctx.idx = idx
        return schemoe_custom_kernel.nccl_stream_acquire(input, idx)

    @staticmethod
    def backward(ctx: Any, grad_output: Tensor) -> Tensor:
        if not AllToAllStatus.initialized:
            return (grad_output, None)
        return (schemoe_custom_kernel.nccl_stream_release(grad_output, ctx.idx), None)
...
class AllToAll2DAsync(torch.autograd.Function):
    ...
    def forward(ctx: Any, input: Tensor) -> Tensor:
        if not AllToAllStatus.initialized:
            return input
        return schemoe_custom_kernel.nccl_all_to_all_2d_async(input)

    @staticmethod
    def backward(ctx: Any, grad_output: Tensor) -> Tensor:
        if not AllToAllStatus.initialized:
            return (grad_output, None)
        return schemoe_custom_kernel.nccl_all_to_all_2d_async(grad_output)


class AllToAllScatterAsync(torch.autograd.Function):
    @staticmethod
    def forward(ctx: Any, input: Tensor, is_compress: bool) -> Tuple[Tensor]:
        if not AllToAllStatus.initialized:
            return (input,)
        ...
        ctx.is_compress = is_compress
        if ctx.is_compress:
            return tuple(schemoe_custom_kernel.nccl_all_to_all_scatter_zfp_async(input, output_shape, ctx.num_split, ctx.num_slices_per_split, False, 8.0))
        return tuple(schemoe_custom_kernel.nccl_all_to_all_scatter_async(input, output_shape, ctx.num_split, ctx.num_slices_per_split, False))
    ...
    def backward(ctx: Any, *grad_output) -> Tensor:
        ...
        if ctx.is_compress:
            return schemoe_custom_kernel.nccl_all_to_all_gather_zfp_async(grad_output, ctx.input_shape, ctx.num_split, ctx.num_slices_per_split, True, 8.0), None
        return schemoe_custom_kernel.nccl_all_to_all_gather_async(grad_output, ctx.input_shape, ctx.num_split, ctx.num_slices_per_split, True), None


class AllToAllGatherAsync(torch.autograd.Function):
    @staticmethod
    def forward(ctx: Any, is_compress, *input) -> Tensor:
        if not AllToAllStatus.initialized:
            return input[0]
        ...
        ctx.is_compress = is_compress
        if ctx.is_compress:
            return schemoe_custom_kernel.nccl_all_to_all_gather_zfp_async(input, output_shape, ctx.num_split, ctx.num_slices_per_split, False, 8.0)
        return schemoe_custom_kernel.nccl_all_to_all_gather_async(input, output_shape, ctx.num_split, ctx.num_slices_per_split, False)
    ...
    def backward(ctx: Any, grad_output: Tensor) -> Tuple[Tensor]:
        if not AllToAllStatus.initialized:
            return (grad_output,)
        if ctx.is_compress:
            return tuple([None, ] + schemoe_custom_kernel.nccl_all_to_all_scatter_zfp_async(grad_output, ctx.input_shape, ctx.num_split, ctx.num_slices_per_split, True, 8.0))
        return tuple([None, ] + schemoe_custom_kernel.nccl_all_to_all_scatter_async(grad_output, ctx.input_shape, ctx.num_split, ctx.num_slices_per_split, True))
    ...
    def forward(ctx, x, input_dim, output_dim):
        if PrimAllToAll2D.LOCAL_SIZE == 0:
            ...
            if PrimAllToAll2D.LOCAL_SIZE == 1:
                logging.warning(
                    "LOCAL_SIZE (> 1) for AllToAll 2DH is not set, please set the correct LOCAL_SIZE variable, or using mpiexec to launch tutel.net")
        ...
        y = y.view(list(y.shape[:input_dim]) + [get_world_size(dist.model_group), get_world_size(dist.data_group), -
                   1] + list(y.shape[input_dim + 1:])).swapaxes(input_dim, input_dim + 1).contiguous().view(y.shape)
        return y
...
class PrimAllToAll(torch.autograd.Function):
    ...
    def transform(input, input_dim, output_dim, group=None, background=False, use_2dh=False):
        ...
        if background:
            ...
            if input_dim == 0:
                reshaped_input = input.view(list(
                    input.shape[:output_dim]) + [world_size, -1] + list(input.shape[output_dim + 1:]))
                reshaped_input = reshaped_input.permute(
                    [output_dim] + list(range(output_dim)) + list(range(output_dim + 1, reshaped_input.dim())))
                output, f_wait = simple_all_to_all(
                    reshaped_input, group, background=True)

                def f_async():
                    f_wait()
                    local_input = RestoreBackward.apply(output, reshaped_input)
                    local_input = local_input.view(
                        [-1] + list(local_input.shape[2:]))
                    return local_input
                ...
                def f_async():
                    ...
                    local_input = RestoreBackward.apply(output, reshaped_input)
                    local_input = local_input.view(
                        [world_size, -1] + list(local_input.shape[1:]))
                    local_input = local_input.permute(list(
                        range(1, input_dim + 1)) + [0] + list(range(input_dim + 1, local_input.dim())))
                    local_input = local_input.contiguous().view(list(
                        local_input.shape[:input_dim]) + [-1] + list(local_input.shape[input_dim + 2:]))
                    return local_input

                return f_async
            else:
                raise Exception(
                    'Unhandle async branch case for flexible all_to_all()')
        ...
        if input_dim == 0:
            ...
            reshaped_input = PrimAllToAll.transform(
                reshaped_input, input_dim, 0, group)
            reshaped_input = swap_axis(
                reshaped_input, 0, output_dim).contiguous()
...
class PrimAllgather(torch.autograd.Function):
    ...
    def zero_scatter(input, scatter_fn, group=None):
        ...
        if full_size % group_size == 0:
            ...
            data = torch.zeros([(full_size + group_size - 1) // group_size *
...
def pre_expert_permute(input, group=None):
    ...
    input = input.contiguous().view(
        [input.shape[0], -1] + list(input.shape[3:]))
    return input
...
def post_expert_permute(input, group=None):
    ...
    if world_size == 1:
        return input
    input = input.view([input.shape[0], world_size, -1] +
                       list(input.shape[2:]))
    input = input.permute([1, 0] + list(range(2, input.dim())))
